...
class User:
    def __init__ (self, first, last, carType, emailAddress):
        self.first = first
        self.last = last
    ...
    def register(self):
        cursor.execute('INSERT OR REPLACE INTO userInfo (firstName, lastName, carType, emailAddress) VALUES (?, ?, ?, ?)', 
                       (self.first, self.last, self.carType, self.emailAddress))
        conn.commit()
